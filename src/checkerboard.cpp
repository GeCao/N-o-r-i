...
public:
    ...
    virtual T eval(const Point2f & uv) override {
        /* to be implemented */
        float uv_x = uv.x() - m_delta.x();
        float uv_y = uv.y() - m_delta.y();
        int number_of_negative = 0;
        if (uv_x <= 0) { number_of_negative += 1; uv_x = -uv_x; }
        if (uv_y <= 0) { number_of_negative += 1; uv_y = -uv_y; }
        if (((int)(uv_x / m_scale.x()) + (int)(uv_y / m_scale.y())) % 2 == 0) {
