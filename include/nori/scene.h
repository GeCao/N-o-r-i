...
public:
    ...
    /// If the scene has environment emitter, return the value of radiance on the environment map (given ray)
    Color3f getEnv(const Ray3f& ray) const {

        Emitter* envLight;
        for (int i = 0; i < m_emitters.size(); i++) {
            if (m_emitters[i]->isEnvEmitter()) {
                envLight = m_emitters[i];
                return envLight->getEnvRadiance(ray);
            }
        }
        return Color3f(0.0f);
    }
