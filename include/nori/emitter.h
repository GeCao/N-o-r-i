...
public:
    ...
    /// Environment emitter or not
    bool isEnvEmitter() { return m_isEnv; }

    /// When the given ray hit on the "sphere", return the value of the pixel
    virtual Color3f getEnvRadiance(const Ray3f& ray) const {
        throw NoriException("Environment emitter not found!");
    }

    /// To get the radius of the scene
    virtual void activate(const Scene* scene) {
        throw NoriException("Environment emitter not found!");
    }
